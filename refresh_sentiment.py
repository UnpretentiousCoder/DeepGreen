...
TRANSCRIPT_CHROMA_PATH = "./chroma_db"
...
def get_refreshed_sentiment(): #THIS FETCHES THE LASTEST DATA
    ...
    if not os.path.exists(CSV_FILE):
        ...
        print(f"\nCSV file '{CSV_FILE}' not found. No periods to be refreshed:")
        return
    else:
        ...
        try:
            ...
            if periods_found:
                ...
                first_transcript_year = latest_first_period[0]
                first_transcript_quarter = latest_first_period[1]

                second_transcript_year = latest_second_period[0]
                second_transcript_quarter = latest_second_period[1]

                print(f"\nLatest periods found in CSV: {first_transcript_year} Q{first_transcript_quarter} and {second_transcript_year} Q{second_transcript_quarter}")
                time.sleep(10)
    ...
    num_companies_per_sector = 10 # You can adjust this number. For now, we'll aim for this many.
...
# --- START OF NEW LOGIC FOR REPLACING LAST COLUMN ---
    ...
    if os.path.exists(csv_file_name):
        ...
        # Remove the last column (excluding 'Sector')
        if len(existing_df.columns) > 1:  # Make sure there's more than just the 'Sector' column
            existing_df = existing_df.iloc[:, :-1]  # Remove last column
        
        # Merge with new data
        final_df = pd.merge(existing_df, new_quarter_df, on='Sector', how='outer')
    else:
        # If no existing file, just use the new data
        final_df = new_quarter_df
